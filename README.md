# SQUiD
Towards ultra-secure, scalable, and queryable genotype-phenotype databases with SQUiD
...
Then, you need to pull the context from the server (make sure the server is running before start this) with `./bin/squid getContext`.
